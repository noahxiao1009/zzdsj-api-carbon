...
from cosight_server.deep_research.services.ais_session_manager import AISSessionManager
...
async def websocket_handler(
    ...
    try:
        # 获取用户ID和历史记录
        session_manager = AISSessionManager()
        user_id = session_manager.get_user_id(websocket_client_key) or "admin"
        
        # 获取用户的会话历史
        conversation_history = await session_manager.get_conversation_history(user_id)
        
        # 获取用户的会话列表
        conversations = await session_manager.get_user_conversations(user_id, 5)
        ...
        if conversation_history:
            ...
            await manager.send_json(history_message, websocket)
            logger.info(f"发送历史记录给用户: {user_id}, 消息数: {len(conversation_history)}, 会话数: {len(conversations)}")
...
async def _send_resp(websocket, cookie, topic, message, lang):
    ...
    # 获取用户ID和历史记录
    session_manager = AISSessionManager()
    user_id = session_manager.get_user_id(cookie_str) or "admin"
    
    # 获取会话历史记录
    conversation_history = await session_manager.get_conversation_history(user_id, topic)
    
    # 保存用户输入的消息
    conversation_id = await session_manager.save_user_message(
        user_id=user_id,
        content=message.get("initData"),
        role="user",
        conversation_id=topic if topic != user_id else None,
        message_type="chat"
    )
    
    params = {
        "content": message.get("initData"),
        "history": conversation_history,  # 使用从数据库获取的历史记录
        "sessionInfo": {
            "locale": lang,
            "sessionId": conversation_id or topic,  # 使用真实的会话ID
            "username": message.get("roleInfo").get("name"),
            "assistantNames": assistants,
            "conversationId": conversation_id  # 添加会话ID供后续使用
        },
        "stream": True,
